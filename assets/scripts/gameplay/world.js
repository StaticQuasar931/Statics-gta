...
export class GameWorld {
  ...
  applySettings(settings) {
    ...
    this.renderer.setScale(1.2 * fidelity);
  ...
  update(delta) {
    ...
    this.pointerWorld = this.renderer.screenToWorld(this.input.pointer.x, this.input.pointer.y);
  ...
  _generateCity() {
    ...
    for (let y = -count / 2; y < count / 2; y += 1) {
      for (let x = -count / 2; x < count / 2; x += 1) {
        ...
        const heightVariance = (Math.abs(x * y) % 50) + 10;
        const tile = { x: x * TILE_SIZE, y: y * TILE_SIZE, district, spriteKey, heightVariance };
